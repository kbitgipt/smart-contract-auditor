...
const AnalysisManager = ({ project, onBack, isAuditor }) => {
  ...
  const [localEditableResults, setLocalEditableResults] = useState(null);
  ...
  useEffect(() => {
    if (editableResults && currentStep === "edit") {
      setLocalEditableResults(JSON.parse(JSON.stringify(editableResults)));
    }
  }, [editableResults, currentStep]);
  ...
  // Fetch project structure and get analysis
  ...
  // Handle static results modifications
  ...
  const handleSaveModifications = async (modifiedResults) => {
    if (!activeAnalysis) return;

    try {
      setLoading(true);
      const result = await ProjectService.modifyResults(
        activeAnalysis.id,
        {
          vulnerabilities: modifiedResults.vulnerabilities,
          summary: modifiedResults.summary,
  ...
  const handleResetModifications = async () => {
    ...
    if (
      !confirm(
        "Are you sure you want to reset all modifications and restore original results?"
      )
    ) {
      return;
    }
    ...
    try {
      setLoading(true);
      const result = await ProjectService.resetModifications(
        activeAnalysis.id,
        token
      );

      if (result && result.success) {
        alert("Original results restored successfully!");

        // Refresh the static results display
        handleViewStaticResults(activeAnalysis.id);
        fetchAnalyses();
      } else {
      ...
      alert("Failed to reset modifications: " + error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddVulnerability = () => {
    if (!localEditableResults) return;
    ...
    setLocalEditableResults({
      ...localEditableResults,
      vulnerabilities: [...localEditableResults.vulnerabilities, newVuln],
    });
  };
  ...
  const handleRemoveVulnerability = (index) => {
    ...
    const newVulns = localEditableResults.vulnerabilities.filter((_, i) => i !== index);
    const newSummary = recalculateSummary(newVulns);
  ...
  const recalculateSummary = (vulnerabilities) => {
    const summary = { total: 0, high: 0, medium: 0, low: 0, informational: 0 };
  ...
  const handleVulnerabilityChange = (index, field, value) => {
    ...
    const newVulns = [...localEditableResults.vulnerabilities];
    newVulns[index][field] = value;
    
    // Auto-recalculate summary if severity changed
    let newSummary = localEditableResults.summary;
    if (field === 'severity') {
      newSummary = recalculateSummary(newVulns);
    }
  ...
  const handleAiEnhancement = async () => {
    ...
    try {
      setLoading(true);
      const result = await ProjectService.performAiEnhancement(
        activeAnalysis.id,
        token
      );
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      setLoading(true);
      const result = await ProjectService.generateReport(
        activeAnalysis.id,
        "html",
        token
      );
      ...
      if (result && result.success) {
        ...
        alert(
          "Report generated successfully! You can now download it from the previous analyses section."
        );
      } else {
    ...
    return (
      <div className="card bg-yellow-50 border-yellow-200">
        <p className="text-yellow-800">
          Advanced analysis features are only available in auditor mode.
        </p>
        <button
          onClick={onBack}
    ...
    <div className="space-y-6">
      <div>
        <h4 className="font-semibold text-gray-900 mb-3">
          {isFoundryProject
            ? "Foundry Project Analysis Configuration"
        ...
        {isFoundryProject && (
          <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            ...
            <p className="text-blue-800 text-sm">
              For Foundry projects, Slither will analyze the entire project
              automatically. You cannot select specific target files - this
              ensures complete dependency analysis.
            </p>
          </div>
        )}
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2 max-h-40 overflow-y-auto border rounded p-2">
                ...
                {[...new Set(detectors.available_detectors)].map(
                  (detector, index) => (
                    <label
                      key={`detector-${detector}-${index}`}
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const newDetectors = e.target.checked
                            ...
                            : slitherOptions.detectors.filter(
                                (d) => d !== detector
                              );
                          setSlitherOptions({
                            ...slitherOptions,
            ...
            <div>
              ...
              <div className="space-y-2">
                {[
                  {
                    key: "exclude_dependencies",
                    label: "Exclude Dependencies",
                    default: true,
                    recommended: true,
                  },
                  {
                    key: "exclude_informational",
                    label: "Exclude Informational",
                    default: false,
                  },
                  {
                    key: "exclude_optimization",
                    label: "Exclude Optimization",
                    default: false,
                  },
                  {
                    key: "exclude_low",
                    label: "Exclude Low Severity",
                    default: false,
                  },
                ].map((option) => (
        ...
        >
          {loading
            ? `Running ${isFoundryProject ? "Foundry" : "Static"} Analysis...`
            : `Start ${isFoundryProject ? "Foundry" : "Static"} Analysis`}
        </button>
      </div>
    </div>
  );
            ...
            <div className="bg-gray-50 border rounded-lg p-4">
              <div className="flex space-x-4 text-sm">
                <span className="text-red-600">
                  High: {activeAnalysis.summary.high}
                </span>
                <span className="text-orange-600">
                  Medium: {activeAnalysis.summary.medium}
                </span>
                <span className="text-yellow-600">
                  Low: {activeAnalysis.summary.low}
                </span>
                <span className="text-blue-600">
                  Info: {activeAnalysis.summary.informational}
                </span>
              </div>
                ...
                {viewMode === "parsed" ? (
                  ...
                  <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-5 gap-4 text-center">
                          <div>
                            <div className="text-xl font-bold text-gray-900">
                              {staticResults.parsed_results?.summary?.total ||
                                0}
                            </div>
                          ...
                          <div>
                            <div className="text-xl font-bold text-orange-600">
                              {staticResults.parsed_results?.summary?.medium ||
                                0}
                            </div>
                          ...
                          <div>
                            <div className="text-xl font-bold text-blue-600">
                              {staticResults.parsed_results?.summary
                                ?.informational || 0}
                            </div>
                    ...
                    <div>
                      <h4 className="font-medium text-gray-900 mb-3">
                        Vulnerabilities (
                        {staticResults.parsed_results?.vulnerabilities
                          ?.length || 0}
                        )
                      </h4>
                      {staticResults.parsed_results?.vulnerabilities?.length >
                      0 ? (
                        <div className="space-y-3">
                          {staticResults.parsed_results.vulnerabilities.map(
                            (vuln, index) => (
                              <div
                                key={index}
  ...
  const renderEditStep = () => {
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h4 className="font-semibold text-gray-900">
            Edit Static Analysis Results
          </h4>
          <div className="space-x-2">
            <button
              onClick={() => handleSaveModifications(localEditableResults)}
              disabled={loading}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-50 border rounded-lg p-4">
                  ...
                  onClick={() =>
                    setLocalEditableResults({
                      ...
                      summary: recalculateSummary(
                        localEditableResults.vulnerabilities
                      ),
                    })
                  }
              ...
              <div className="grid grid-cols-5 gap-4">
                {["total", "high", "medium", "low", "informational"].map(
                  (severity) => (
                    <div key={severity}>
                      ...
                      <input
                        type="number"
                        min="0"
                        value={localEditableResults.summary[severity]}
                        onChange={(e) =>
                          setLocalEditableResults({
                            ...localEditableResults,
                            summary: {
                              ...localEditableResults.summary,
            ...
            <div className="bg-gray-50 border rounded-lg p-4">
              <div className="flex items-center justify-between mb-3">
                <h5 className="font-medium">
                  Vulnerabilities ({localEditableResults.vulnerabilities.length}
                  )
                </h5>
                <button
              ...
              <div className="space-y-4 max-h-96 overflow-y-auto">
                {localEditableResults.vulnerabilities.map((vuln, index) => (
                  <div
                    key={vuln.id || index}
                ...
                {localEditableResults.vulnerabilities.length === 0 && (
                  <div className="text-center py-8 text-gray-500">
                    No vulnerabilities. Click "Add Vulnerability" to create one.
                  </div>
                )}
              </div>
            </div>
    ...
    <div className="space-y-4">
      ...
      {activeAnalysis && (
        ...
        >
          {activeAnalysis.ai_enhanced === false ? (
            <div>
              <p className="text-yellow-800 mb-2">
                ⚠️ AI enhancement encountered issues but static analysis is
                complete
              </p>
          ...
          {/* AI recommendations */}
          {activeAnalysis.ai_recommendations &&
            activeAnalysis.ai_recommendations.length > 0 && (
              <div className="mt-4">
                ...
                <div className="space-y-2 max-h-40 overflow-y-auto">
                  {activeAnalysis.ai_recommendations.map((rec, index) => (
                    ...
                    >
                      <div className="flex items-start">
                        <div
                          className={`w-6 h-6 text-white rounded-full flex items-center justify-center text-xs font-bold mr-3 mt-0.5 ${
                            rec.includes("failed") || rec.includes("error")
                              ? "bg-red-600"
                              : "bg-blue-600"
                          }`}
          ...
          {activeAnalysis.vulnerabilities &&
            activeAnalysis.vulnerabilities.length > 0 && (
              <div className="mt-4">
                <h5 className="font-medium text-green-900 mb-2">
                  AI Enhanced Vulnerabilities (
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto">
                  ...
                  {activeAnalysis.vulnerabilities.length > 3 && (
                    <p className="text-xs text-gray-500 text-center">
                      ...and {activeAnalysis.vulnerabilities.length - 3} more
                      vulnerabilities
                    </p>
                  )}
                </div>
      ...
      <div className="card">
        ...
        <div className="flex items-center space-x-4 mb-6">
          {[
            {
              key: "configure",
              label: isFoundryProject ? "Configure Slither" : "Configure",
            },
        ...
        {currentStep === "report" && (
          <div className="text-center py-8">
            ...
            <p className="text-gray-600">
              Your {isFoundryProject ? "Foundry project" : "analysis"} report
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex space-x-4 mt-2 text-xs">
                      ...
                      <span className="text-blue-600">
                        Info: {analysis.summary.informational}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <div className="flex space-x-2">
                      ...
                      {analysis.report_available && (
                        <button
                          onClick={() =>
                            ProjectService.viewReport(analysis.id, token)
                          }
                          className="px-3 py-1 bg-green-600 text-white text-sm rounded hover:bg-green-700"
